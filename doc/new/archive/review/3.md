...
**审查日期:** 2026-01-20  
**审查对象:** repo/ 下实现  
**对照规范:** doc/new/3命令规范.md  
**审查者:** opencode

## 概述

根据《命令规范》文档，对当前代码库实现进行全面审查。规范定义了以命令为中心的执行模型、CLI与库职责分离、事件驱动日志、模块化设计等核心原则。本报告评估实际实现与规范的符合程度，识别差距与改进建议。
...
- **并发调度**: CLI 支持步骤执行但未提供统一线程池管理，并发处理在库函数内部实现。
...
| 规范示例 | 实现对应 | 状态 |
|----------|----------|------|
| `work.scanwork(workdir) -> Iterable[WorkItem]` | `publish_logic` 返回待处理项列表 | ✅ 符合 |
| `metadata.diff_items(items) -> Iterable[DiffItem]` | `publish_logic` 内部计算字段级差异 | ⚠️ 部分符合 |
| `audio.hashandstore(item, ctx) -> StoreResult` | `execute_publish` 处理哈希和存储 | ✅ 符合 |
| `audio.process_cover(item, ctx) -> CoverResult` | `compress_images_step` 处理封面压缩 | ✅ 符合 |
| `hash.verifylocalandremote(audiooid, ctx) -> VerifyResult` | `verify_logic` 提供校验功能 | ✅ 符合 |
| `metadata.addorupdateandcommit(entries, message, ctx) -> CommitResult` | `commit_step` 提供提交功能 | ✅ 符合 |
| `objectstore.ensurelocal(oids, ctx) -> EnsureResult` | `sync_logic` 提供同步功能 | ✅ 符合 |
| `sync.plan(localindex, remoteindex) -> SyncPlan` | `analyze_sync_diff` 生成同步计划 | ✅ 符合 |
| `sync.execute(plan, ctx) -> SyncResult` | `execute_sync` 执行同步 | ✅ 符合 |
| `verify.check(item, ctx) -> VerifyResult` | `verify_logic` 执行校验 | ✅ 符合 |
| `cleanup.find_orphans(ctx) -> List[Oid]` | `cleanup_logic` 计算孤立对象 | ✅ 符合 |
| `download.fetch_metadata(url) -> MetadataPreview` | `download_logic` 支持元数据获取 | ✅ 符合 |
| `download.run(url, out_dir, ctx) -> DownloadResult` | `execute_download` 执行下载 | ✅ 符合 |
...
| 外部工具：若调用 yt-dlp 等外部工具，库应捕获其输出并映射为事件 | 在 `download` 命令中调用 yt-dlp，输出映射为事件 | ✅ 符合 |
...
| 检查项 | 规范要求 | 实现现状 | 状态 |
|--------|----------|----------|------|
| 参数支持 | `--changed-only`、`--preview` | 完全支持 | ✅ 符合 |
| 工作流步骤 | 7个步骤（scan→diff→preview→hash→compress→verify→commit） | 6个步骤（合并验证步骤） | ⚠️ 部分符合 |
| 事件发射 | 各阶段 emit 事件 | 完整事件发射 | ✅ 符合 |
| 失败策略 | `on_error=stop` | 设置为 `stop` | ✅ 符合 |
| 锁策略 | 写 metadata 前获取写锁 | `requires_lock=True` | ✅ 符合 |
| 并发处理 | 支持批量并发 | 未实现并发 | ❌ 不符合 |

**具体差距:**
1. 工作流步骤顺序与规范不完全一致，压缩封面被拆分为单独步骤
2. 缺少独立的 `validateandverify` 步骤，合并到通用 verify 步骤
3. 未实现并发处理大文件批量发布
...
| 检查项 | 规范要求 | 实现现状 | 状态 |
|--------|----------|----------|------|
| 参数支持 | `<query>`、`--missing`、`--force`、`--limit`、`--search-field`、`--line` | 缺少 `--search-field`、`--line` | ⚠️ 部分符合 |
| 工作流步骤 | 4个步骤（query→conflict→fetch→embed） | 2个步骤（合并功能） | ⚠️ 部分符合 |
| 事件发射 | 各阶段 emit 事件 | 基本事件发射 | ✅ 符合 |
| 失败策略 | 冲突时默认退出 | 简单冲突检查 | ⚠️ 部分符合 |
| 锁策略 | 需要锁 | `requires_lock=True` | ✅ 符合 |
| 并发处理 | 支持并发下载 | 未实现并发 | ❌ 不符合 |

**具体差距:**
1. 缺少 `--search-field`、`--line` 参数支持
2. 冲突检查逻辑简单（仅检查 work 目录非空），未实现规范的连锁调用 `publish --preview`

### 6.3 release 命令

| 检查项 | 规范要求 | 实现现状 | 状态 |
|--------|----------|----------|------|
| 参数支持 | `--mode`、`--force`、`--line`、`--workers` | 缺少 `--force`、`--workers` | ⚠️ 部分符合 |
| 工作流步骤 | 6个步骤（pull→prepare→sync→generate→verify→summary） | 3个步骤（合并功能） | ⚠️ 部分符合 |
| 事件发射 | 各阶段 emit 事件 | 完整事件发射 | ✅ 符合 |
| 失败策略 | `on_error=continue` | 设置为 `stop` | ❌ 不符合 |
| 锁策略 | 需要锁 | `requires_lock=True` | ✅ 符合 |
| 并发处理 | 支持 `--workers` 并发 | 未实现并发 | ❌ 不符合 |

**具体差距:**
1. 缺少 `--force`、`--workers` 参数支持
2. 失败策略错误设置为 `stop` 而非 `continue`
3. 未实现并发生成，缺少 `--workers` 参数功能

### 6.4 sync 命令

| 检查项 | 规范要求 | 实现现状 | 状态 |
|--------|----------|----------|------|
| 参数支持 | `--direction`、`--dry-run`、`--timeout`、`--workers` | 完全支持 | ✅ 符合 |
| 别名支持 | `push`、`pull` 作为别名 | 完全支持 | ✅ 符合 |
| 工作流步骤 | 4个步骤（scan→diff→execute→summary） | 完全符合 | ✅ 符合 |
| 事件发射 | 各阶段 emit 事件 | 完整事件发射 | ✅ 符合 |
| 失败策略 | 传输失败按重试策略处理 | 支持重试和指数退避 | ✅ 符合 |
| 锁策略 | 不需要锁 | `requires_lock=False` | ✅ 符合 |
| 并发处理 | 支持 `--workers` 并发 | 完全实现并发 | ✅ 符合 |
...
| 检查项 | 规范要求 | 实现现状 | 状态 |
|--------|----------|----------|------|
| 参数支持 | `--mode`、`--delete` | 缺少 `--delete` | ⚠️ 部分符合 |
| 工作流步骤 | 4个步骤（collect→compute→delete→summary） | 缺少 `delete` 步骤 | ⚠️ 部分符合 |
| 事件发射 | 各阶段 emit 事件 | 基本事件发射 | ✅ 符合 |
| 失败策略 | 校验失败记录为 error 或 warn | 记录错误但缺少删除机制 | ⚠️ 部分符合 |
| 锁策略 | 不需要锁 | `requires_lock=False` | ✅ 符合 |
| 并发处理 | 支持并发校验 | 未实现并发 | ❌ 不符合 |

**具体差距:**
1. 缺少 `--delete` 参数支持，无法自动删除校验失败文件
2. 未实现 `optional_delete` 步骤
3. 缺少回收站功能（规范要求移动到回收站）

### 6.6 cleanup 命令

| 检查项 | 规范要求 | 实现现状 | 状态 |
|--------|----------|----------|------|
| 参数支持 | `--mode`、`--confirm` | 完全支持 | ✅ 符合 |
| 工作流步骤 | 4个步骤（build→scan→compute→report） | 完全符合 | ✅ 符合 |
| 事件发射 | 各阶段 emit 事件 | 完整事件发射 | ✅ 符合 |
| 失败策略 | 默认仅报告；删除需 `--confirm` | 完全符合 | ✅ 符合 |
| 锁策略 | 需要锁 | `requires_lock=True` | ✅ 符合 |
| 并发处理 | 支持并发清理 | 未实现并发 | ❌ 不符合 |

**具体差距:**
1. 远端清理使用直接 SSH 命令而非 TransportAdapter
2. 未实现并发清理大数量孤立文件
...
| 检查项 | 规范要求 | 实现现状 | 状态 |
|--------|----------|----------|------|
| 参数支持 | `<URL>`、`--batch-file`、`--fetch`、`--no-preview` | 缺少 `--fetch`、`--no-preview` | ⚠️ 部分符合 |
| 工作流步骤 | 4个步骤（fetch→download→compress→verify） | 缺少 compress 和 verify 步骤 | ⚠️ 部分符合 |
| 事件发射 | 各阶段 emit 事件 | 基本事件发射 | ✅ 符合 |
| 失败策略 | 批量模式继续处理其他项 | 支持批量继续 | ✅ 符合 |
| 锁策略 | 不需要锁 | `requires_lock=False` | ✅ 符合 |
| 并发处理 | 支持并发下载 | 未实现并发 | ❌ 不符合 |

**具体差距:**
1. 缺少 `--fetch`、`--no-preview` 参数支持
2. 未集成 `compresscover` 和 `verifyandsync` 到工作流
3. 下载后未自动触发元数据提交
...
| 检查项 | 规范要求 | 实现现状 | 状态 |
|--------|----------|----------|------|
| 参数支持 | `<query>`、`--search-field`、`--line`、`--read`、`--missing`、`--filter`、`--limit` | 缺少 `--filter`、`--limit` | ⚠️ 部分符合 |
| 工作流步骤 | 2个步骤（query→compute） | 完全符合 | ✅ 符合 |
| 事件发射 | 各阶段 emit 事件 | 完整事件发射 | ✅ 符合 |
| 失败策略 | 查询参数错误返回 error | 参数错误返回错误消息 | ✅ 符合 |
| 锁策略 | 不需要锁 | `requires_lock=False` | ✅ 符合 |
| 并发处理 | 支持并发分析 | 未实现并发 | ❌ 不符合 |

**具体差距:**
1. 缺少 `--filter`、`--limit` 参数支持
2. `--read` 参数命名为 `--fields`，名称不一致

### 6.9 compress_images 命令

| 检查项 | 规范要求 | 实现现状 | 状态 |
|--------|----------|----------|------|
| 参数支持 | `--size <n>`（支持单位） | 使用 `--min-size-kb`，名称和单位不符 | ❌ 不符合 |
| 工作流步骤 | 3个步骤（scan→compress→update） | 完全符合 | ✅ 符合 |
| 事件发射 | 各阶段 emit 事件 | 完整事件发射 | ✅ 符合 |
| 失败策略 | 压缩失败记录 warn 并继续 | 支持失败继续 | ✅ 符合 |
| 锁策略 | 需要锁 | `requires_lock=True` | ✅ 符合 |
| 并发处理 | 支持并发压缩 | 未实现并发 | ❌ 不符合 |

**具体差距:**
1. 参数命名和单位不符合规范，缺少 `--size` 参数
2. 未实现单位自动解析（kb/mb/gb）
...
| 3. CLI 骨架实现 命令注册、步骤引擎、事件监听、日志写入 | ⚠️ 部分完成 - 缺少日志持久化 |
...
| libgitmusic/commands/ 所有命令实现 | ✅ 存在 |
...
### 总体评价
当前实现 **基本符合** 命令规范架构，核心模块完整，事件系统健全，CLI 功能齐全。主要差距集中在 **日志持久化**、**传输层完整性校验**、**参数支持不全** 和 **测试覆盖**。

### 关键差距
1. **日志持久化缺失** - 事件流未写入文件，不符合规范要求的 `logs/<command>-TIMESTAMP.jsonl`。
2. **TransportAdapter 不完整** - 缺少远端 SHA256 校验和重试策略。
3. **参数支持不全** - 多个命令缺少规范要求的参数（如 `--search-field`、`--delete`、`--workers` 等）。
4. **测试缺失** - 无单元测试或集成测试。
5. **并发处理不足** - 仅 sync 命令实现完整并发，其他命令未充分利用并发机制。
6. **错误处理未完全实现** - `on_error` 策略未实际应用，部分命令失败策略设置错误。

### 高优先级建议
1. **实现日志持久化** - 在 CLI 中增加事件写入 JSONL 文件的功能，存储到 `logs/` 目录。
2. **增强 TransportAdapter** - 添加上传后的远端哈希校验，实现重试机制配置。
3. **补全命令参数** - 为 checkout、verify、download、analyze、compress_images 等命令添加缺失的参数支持。
4. **统一错误策略** - 根据 `on_error` 值实现 stop/continue/notify 行为，修正 release 命令的错误策略。
5. **创建测试套件** - 为库模块添加单元测试，为命令链添加集成测试。

### 中优先级建议
6. **完善并发调度** - 在 CLI 中提供线程池，允许步骤声明并发参数，为 publish、checkout、release 等命令添加并发支持。
7. **补充工作流步骤** - 完善 publish 命令的字段级 diff 显示和预览确认，完善 checkout 命令的冲突处理逻辑。
8. **补充文档** - 创建 command-spec.md 和操作手册。

### 低优先级建议
9. **实现日志轮转** - 当日志持久化后，增加归档策略。
10. **添加监控导出** - 将 verify 结果导出到监控系统。
11. **优化参数解析** - 实现 compress_images 命令的单位自动解析。
